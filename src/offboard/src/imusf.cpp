...
int main(int argc, char **argv)
{
    ...
    ros::Publisher ahrs_pub = nh.advertise<geometry_msgs::Quaternion>
             ("imusf", 1000);
    ros::Publisher pub_co = nh.advertise<std_msgs::Float64MultiArray>
          ("rotateM", 400);
    ...
    ros::Publisher imuangle_pub = nh.advertise<geometry_msgs::Vector3>
                      ("imu_angle", 1000);
    ros::Publisher test_pub = nh.advertise<geometry_msgs::Vector3>
                      ("test", 1000);
  ...
  geometry_msgs::Vector3 imuangle;
    ...
    while(ros::ok()){
      std_msgs::Float64MultiArray rot;
        quatfinal.x=quat.x();
        quatfinal.y=quat.y();
        quatfinal.z=quat.z();
        quatfinal.w=quat.w();
...
M(0,0)=cos(angle.y)*cos(angle.z);
M(0,1)=sin(angle.x)*sin(angle.y)*cos(angle.z)-cos(angle.x)*sin(angle.z);
M(0,2)=sin(angle.x)*sin(angle.z)+cos(angle.x)*sin(angle.y)*cos(angle.z);
M(1,0)=cos(angle.y)*sin(angle.z);
M(1,1)=cos(angle.x)*cos(angle.z)+sin(angle.x)*sin(angle.y)*sin(angle.z);
M(1,2)=cos(angle.x)*sin(angle.y)*sin(angle.z)-sin(angle.x)*cos(angle.y);
M(2,0)=-sin(angle.z);
M(2,1)=sin(angle.x)*cos(angle.y);
M(2,2)=cos(angle.x)*cos(angle.y);

for(int i=0;i<3;i++)
        {
          for(int j=0;j<3;j++)
          {
            rot.data.push_back(M(i,j));
          }
        }

Eigen::Matrix< float, 3, 1> teste=M.eulerAngles(0,1,2);
	test.x=teste[0];
	test.y=teste[1];
	test.z=teste[2];
